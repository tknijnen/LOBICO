function [optfun,cons,consub,lb,ub,ctype] = DNF_CPLEX(X,Y,W,K,M,lambda,sens,spec,addcons)
...
for n = 1:N;
    for k = 1:K;
        for p = 1:P;
            if X(n,p)==1;
                cons(conr+(n-1)*P*K+(k-1)*P+p,[2*P*K+(k-1)*N+n P*K+(k-1)*P+p]) = 1;
            else
                cons(conr+(n-1)*P*K+(k-1)*P+p,[2*P*K+(k-1)*N+n (k-1)*P+p]) = 1;
            end
...
%         consub(conr+(k-1)*N+n) = P-1-length(PosSet)-length(NegSet);
