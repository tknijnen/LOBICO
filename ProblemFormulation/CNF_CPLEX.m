function [optfun,cons,consub,lb,ub,ctype] = CNF_CPLEX(X,Y,W,K,M,lambda,sens,spec,addcons)
...
for n = 1:N;
    for k = 1:K;
        for p = 1:P;
            if X(n,p)==1;
                cons(conr+(n-1)*P*K+(k-1)*P+p,[2*P*K+(k-1)*N+n]) = -1;
                cons(conr+(n-1)*P*K+(k-1)*P+p,[(k-1)*P+p]) = 1;
            else
                cons(conr+(n-1)*P*K+(k-1)*P+p,[2*P*K+(k-1)*N+n]) = -1;
                cons(conr+(n-1)*P*K+(k-1)*P+p,[P*K+(k-1)*P+p]) = 1;
            end
            consub(conr+(n-1)*P*K+(k-1)*P+p) = 0;
        end
    end
end
conr = P*K+N*P*K;           
...
for n = 1:N;
    for k = 1:K;
        PosSet = find(X(n,:)==1);
        NegSet = find(X(n,:)==0);
        cons(conr+(k-1)*N+n,[(k-1)*P+PosSet P*K+(k-1)*P+NegSet]) = -1;
